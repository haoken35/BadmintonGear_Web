...
export default function AdminProductItem({ product, onCheck, onDelete, low }) {
    return (
        <tr className='bg-(--surface) shadow-md border-b border-(--border) text-(--text2) hover:bg-(--surface2)'>
            {!low && (
                <td>
                    <input
                        type='checkbox'
                        className='w-5 h-5 accent-(--primary) ml-5'
            ...
            <td>{product.brand}</td>
            <td>{product.cate ? product.cate.name : ""}</td>
            <td>{product.quantity}</td>
            <td>{Number(product.price).toLocaleString()} VND</td>
            <td className="py-4 text-center align-middle">
                <div className={`inline-flex justify-center items-center px-2 py-1 rounded-full text-center
                        ${product.quantity === 0 ? "text-[#F04438] bg-[#FEEDEC]" :
                        (product.quantity < 10 ? "text-[#D97706] bg-[#FFF7ED]" : "text-[#0D894F] bg-[#E7F4EE]")}`}>
            ...
            {!low && (
                <td >
                    <div className='flex justify-center items-center gap-3'>
                        <button onClick={() => window.location.href = `/admin/productdetail?id=${product.id}&&mode=view`}>
