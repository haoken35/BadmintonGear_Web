    ...
    return (
        <tr className='bg-white shadow-md border-b border-[#F0F1F3]'>
            ...
            <td className='flex justify-center items-center py-4'>
                <div className={`w-fit px-2 py-1 rounded-full ${promotion.status === 3 ? "text-[#0D894F] bg-[#E7F4EE]" : "text-[#F04438] bg-[#FEEDEC]"}`}>
                    {promotion.status === 0 ? "Expired" :
                        (promotion.status === 1 ? "Disabled" : (promotion.status === 2 ? "Out of turn" : "Active"))}
                </div>
            </td>
            <td >
                <div className='flex justify-center items-center gap-3'>
                    <button onClick={() => window.location.href = `/promotiondetail?id=${promotion.id}&&mode=view`}>
                    ...
                    <button onClick={() => window.location.href = `/promotiondetail?id=${promotion.id}&&mode=edit`}>
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
