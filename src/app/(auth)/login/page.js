...
export default function Login() {
    ...
    return (
        <div className="w-full h-full">
            <div className="flex items-center my-20 mx-auto h-auto w-fit bg-(--surface) rounded-xl text-montserrat">
                ...
                <div className="ml-10 mr-10 w-fit gap-5 ">
                    <form id="login-form" className="w-fit">
                        ...
                        <input id="username" type="text" placeholder="Username" className="w-4/5 p-2 mt-5 mr-5 border-b border-(--border) text-(--text) bg-(--surface2) focus:border-(--primary) outline-none" required />
                        <input id="password" type="password" placeholder="Password" className="w-4/5 p-2 mt-5 mr-5 border-b border-(--border) text-(--text) bg-(--surface2) focus:border-(--primary) outline-none" required />
                        {error && (<div className="text-red-500 text-sm mt-5">{error}</div>)}
                        <div className="flex items-center mt-10 w-4/5 relative">
                            <button type="submit" onClick={handleLogin} className="pl-10 pr-10 pt-2 pb-2 rounded-xs bg-(--primary) text-white font-montserrat" >Log in</button>
                            <a className="absolute right-0 text-(--primary) text-xs" href="/forgotpassword">Forget Password?</a>
                        </div>
                    </form>
