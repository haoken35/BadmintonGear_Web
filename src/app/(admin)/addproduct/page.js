            ...
            <div className='flex justify-between items-end'>
                <div>
                    ...
                    <div id="roadmap" className="flex items-center mt-2">
                        ...
                        <a className="text-(--primary)" href="/productlist">Product List</a>
                        <label className="ml-3 mr-3">
                            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path fillRule="evenodd" clipRule="evenodd" d="M6.59467 3.96967C6.30178 4.26256 6.30178 4.73744 6.59467 5.03033L10.5643 9L6.59467 12.9697C6.30178 13.2626 6.30178 13.7374 6.59467 14.0303C6.88756 14.3232 7.36244 14.3232 7.65533 14.0303L12.4205 9.26516C12.5669 9.11872 12.5669 8.88128 12.4205 8.73484L7.65533 3.96967C7.36244 3.67678 6.88756 3.67678 6.59467 3.96967Z" fill="#A3A9B6" />
                            </svg>
                        </label>
                        <a className="text-(--muted)" href="/addproduct">Add Product</a>
                    </div>

                </div>
                <div className='flex gap-3'>
                    <button className='border border-(--border) text-(--muted) px-4 py-2 rounded-md flex gap-2 items-center cursor-pointer'
                        ...
                        Cancel</button>
                    <button className='bg-(--primary) text-white px-4 py-2 rounded-md flex gap-2 items-center cursor-pointer disabled:opacity-55 disabled:cursor-not-allowed'
                ...
                <div className='w-2/3 '>
                    <div className='bg-(--surface) shadow-md rounded-lg p-5'>
                        ...
                        <div className='mt-2 gap-1'>
                            ...
                            <textarea
                                className='border border-(--border) bg-(--surface2) text-(--text) rounded-md w-full px-3 py-2 resize-none  outline-none'
                    ...
                    <div className='bg-(--surface) shadow-md rounded-lg p-5 mt-5'>
                        ...
                        <div
                            className={`flex flex-col gap-3 justify-center items-center py-4 bg-(--surface2) rounded-md border border-dashed border-(--border)
                        ...
                        >
                            ...
                            <div className='rounded-full p-2 bg-[#FBE3CA] border-3 border-[#EFEFFD] w-fit flex flex-col items-center justify-center gap-2'>
                                ...
                                <div>
                                    <label htmlFor='upload-image' className='text-(--primary) bg-(--primary)/20 rounded-lg px-4 py-2'>Add Image</label>
                                    <input
                                        id='upload-image'
                                        onChange={handleImageUpload}
                                        type="file"
                                        accept="image/*"
                                        className='border border-(--border) bg-(--surface2) rounded-md w-full px-3 py-2 mt-2 hidden'
                                        multiple
                                    />
                                </div>
                            </div>
                    ...
                    <div className='bg-(--surface) shadow-md rounded-lg p-5 mt-5'>
                        ...
                        <div className='mt-2 gap-1'>
                            ...
                            <div className='border border-(--border) bg-(--surface2) text-(--text) rounded-md w-full px-3 py-2 flex items-center gap-2'>
                                <input
                                    type="number"
                ...
                <div className='w-4/15'>
                    <div className='bg-(--surface) shadow-md rounded-lg p-5 w-full'>
                        ...
                        <div className=' gap-1 mt-2'>
                            ...
                            <select
                                className='w-full border border-(--border) bg-(--surface2) text-(--text) rounded-md px-4 py-2 outline-none '
                                value={selectedCategory}
                                onChange={e => setSelectedCategory(e.target.value)}
                            >
                                <option value="0" disabled>Select Category</option>
                                {category.map((item, index) => (
                                    <option className='text-(--text) bg-(--surface2)' key={index} value={item.id}>{item.name}</option>
                                ))}
                            </select>
