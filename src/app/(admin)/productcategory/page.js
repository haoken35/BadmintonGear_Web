...
export default function ProductCategory() {
        ...
        try {
            ...
            if (response && Array.isArray(response)) {
                const updatedCategories = response.map(category => {
                    const productCount = productResponse.filter(p => p.categoriesid === category.id).length;
                    return {
                        ...category,
                        isChecked: false,
    ...
    return (
        <div className='px-2 py-5'>
            <div className='flex justify-between items-end'>
                <div>
                    ...
                    <div id="roadmap" className="flex items-center mt-2">
                        ...
                        <a className="text-(--text2)" href="/productcategory">Product Category</a>
                    </div>
                </div>
                <div className='flex gap-3'>
                    <button className='bg-[#FBE3CA] text-(--primary) px-4 py-2 rounded-md flex gap-2 items-center'
                    ...
                    {isCheck && (
                        <button className='bg-(--primary) text-white px-4 py-2 rounded-md flex gap-2 items-center' onClick={handleDeleteCategories}>
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className='flex gap-1 items-center bg-(--surface) rounded-md px-4 border border-(--border) mt-5 w-fit'>
            ...
            <div className=' shadow-md rounded-md border border-(--border) mt-5'>
                <table className='w-full py-2 rounded-md overflow-hidden '>
                    <thead className='bg-(--surface2) font-medium border-b border-(--border)'>
                        <tr className='text-left text-(--text) font-semibold rounded-md'>
                            <th>
                                <input
                                    type='checkbox'
                                    className='w-5 h-5 accent-(--primary) ml-5 my-5'
